...
val passwordsList: List<String> = fileinput.split("\n") // [6-10 s: snkscgszxsssscss, 6-7 b: bbbbbxkb, 2-4 n: nnnjn, 1-2 j: jjjj, ]
var validPasswordsCount = passwordsList.filter { passwords -> isValidPassword(passwords) }
                                     .count()

print("validPasswordsCount=${validPasswordsCount}")


fun isValidPassword(completePassword: String): Boolean {
    val splitPassword =  completePassword.split(":")
    val password: String = splitPassword[1].filter { !it.isWhitespace()  }
    val letter: Char = splitPassword[0].split(" ")[1].removePrefix(" ").first()
    val min: Int = splitPassword[0].split(" ")[0].split("-")[0].toInt()
    val max: Int =  splitPassword[0].split(" ")[0].split("-")[1].toInt()
    //  6-10 s: snkscgszxsssscss
    val lowerIndex = min-1
    val upperIndex = max-1

    val aValid: Boolean =  password[lowerIndex].equals(letter) and !password[upperIndex].equals(letter)
    val bValid: Boolean = !password[lowerIndex].equals(letter) and password[upperIndex].equals(letter)
    return aValid xor bValid
}
