...
//PART 1
//val allPasswords = fileInput .trim()
//        .split("\n\n")
...
//    val separatedPassword: List<String> = password.trim().split(":"," ","\n")
//    var requiredFields = mutableMapOf<String,String>()
//
//    for (i in separatedPassword.indices step 2){
//        requiredFields[separatedPassword[i]] = separatedPassword[i+1]
//    }
////    val isValid:Boolean = validateNumFields(requiredFields["byr"],"2002","1920")
////    print(requiredFields)
//    print("\n")
//
//    return requiredFields.containsKey("byr") && requiredFields.containsKey("iyr") && requiredFields.containsKey("eyr")
//            && requiredFields.containsKey("hgt")  && requiredFields.containsKey("hcl")  && requiredFields.containsKey("ecl")
//            && requiredFields.containsKey("pid")
//}
//
//fun validateNumFields(value: String?,max :String , min:String): Boolean {
...
fun validPassword(password: String): Boolean {
    ...
    return validateNumFields(requiredFields["byr"],2002,1920,4)
            && validateNumFields(requiredFields["iyr"],2020,2010,4)
            && validateNumFields(requiredFields["eyr"],2030,2020,4)
            && parseHGT(requiredFields["hgt"])
            && validateHCL(requiredFields["hcl"])
            && validateECL(requiredFields["ecl"])
            && validatePID(requiredFields["pid"])
}

fun validateNumFields(value: String?,max :Int , min:Int,maxLength:Int): Boolean {
    if (value == null) return false
    if(value.length!=maxLength) return false
    val numValue = value.toIntOrNull() ?: return false
    return numValue>=min && numValue<=(max)
}

fun parseHGT(hgt: String?): Boolean {
    if(hgt==null) return false
    val regex = """(\d+)([a-zA-z]+)""".toRegex()
    val matched = regex.matchEntire(hgt) ?: return false
    val (numberPart,unitPart) = matched.destructured
    val number = numberPart.toInt()

    if(unitPart.equals("cm")) {
        return number in 150..193
    }
    return number in 59..76
}

fun validateHCL(color: String?): Boolean {
    if (color==null) return false
    if(color.get(0) != '#') return false
    val regex = """^#[0-9a-f]{6}$""".toRegex()
    return regex.matches(color)
}

fun validateECL(eyeColor: String?): Boolean {
...
fun validatePID(passportID: String?): Boolean {
    if(passportID==null) return false
    val regex = """^\d{9}$""".toRegex()
    return regex.matches(passportID)
}
